...
code = open("wmma_example.cu").read()
prog = CUDAProgram(device, "wmma_example", compiler.compile(code))

#global_size, local_size = [(N//16)//4, (N//16)//4, 1], [32, 1, 1]
global_size=[512, 256, 1]
local_size=[8, 2, 2]
tm = min([prog(c, a, b, global_size=global_size, local_size=local_size, wait=True) for _ in range(20)])
print(f"{N*N:10d} {tm*1e6:9.2f} us, would be {FLOPS*1e-9/tm:9.2f} GFLOPS matmul, {BW*1e-9/tm:.2f} GB/s")
cudaalloc.copyout(flat_mv(nc.data), c)
np.testing.assert_allclose(na.astype(np.float32) @ nb.astype(np.float32), nc.reshape(N,N), atol=0.2)